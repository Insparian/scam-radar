...
- The first deployed build exposed a stale/invalid publishable key. The protected GitHub environment was corrected to the current public key and run `35187327856` rebuilt and redeployed the same protected branch; a fake-credential probe then reached Auth and failed with the expected `invalid_credentials` response.
- The interactive smoke test passed on 2026-09-17: a Cloudflare account member passed Access, the mapped Supabase reviewer signed in, the bootstrap RPC returned an empty `0 OPEN` queue, sign-out cleared the Supabase session, and a direct queue visit redirected back to login. No real evidence or decision row was created or changed.
