...
| F4 | Reviewer browser → Supabase | Publishable key, login credentials/session metadata, narrow queue payload, evidence choices, exception action and decision note | Human exception decision or V0.1 shadow confirmation | Sixth migration and protected preview active; unauthenticated Access boundary verified; authenticated smoke pending; no real review rows | Cloudflare Access, Supabase Auth, RPC-only browser data access, enabled admin row, row version, transactional RPC, distinct human audit, unconfigured build fails closed |
