...
| `@supabase/supabase-js` | `2.116.0`, exact direct dependency in the npm lock | Reviewer password Auth, session refresh, and narrow review RPCs | First-party Supabase SDK; npm metadata last modified 2026-09-07 | Handwritten `fetch` for Auth/PostgREST | Loaded only by `/admin`; public content/search does not initialize Supabase; see ADR-009 |
