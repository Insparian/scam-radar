...
7. Keep all three kill switches off.
...
Stop and ask Rui before any paid plan, new outbound data path, production write not described above, destructive migration, public upload, DNS edit, wider credential scope, or a forward migration adding an exact policy/gate tuple to the live `safe_to_automate` allowlist.
